	...
	public function detail(Request $request){
		...
		if ($request->session->getSession('valid_user') == true){
			...
			if($FORM_SUBMITTED_BY_METHOD == 'POST'){
				...
				$query = 'SELECT contact FROM user_contact WHERE id =' . $id . ' and id_user =' . $id_user;
				$resultQuery = $database->selectFromTable($query);
				$template = $this->container->get('TemplateTwig');
				$array = array( 'contacts' => $resultQuery);
	...
	public function edit(Request $request){
		...
		if ($request->session->getSession('valid_user') == true){
			...
			if($FORM_SUBMITTED_BY_METHOD == 'POST'){
				...
				$id = $request->cleanData($request->post->getParam('id'));
				$id_user = $request->session->getSession('user_ref');
				$query = 'SELECT id, contact FROM user_contact WHERE id =' . $id . ' and id_user =' . $id_user;
				$resultQuery = $database->selectFromTable($query);
				$template = $this->container->get('TemplateTwig');
				$array = array( 'contacts' => $resultQuery);
	...
	public function update(Request $request){
		...
		if ($request->session->getSession('valid_user') == true){
			...
			if($FORM_SUBMITTED_BY_METHOD == 'POST'){
				...
				$id_user = $request->session->getSession('user_ref');
				$resultQuery = $database->updateTable('UPDATE user_contact SET contact = :contact WHERE id = :id and id_user = :id_user', array('contact' => $newData, 'id' => $id, 'id_user' => $id_user));
				$template = $this->container->get('TemplateTwig');
	...
	public function erase(Request $request){
		...
		if ($request->session->getSession('valid_user') == true){
			...
			if($FORM_SUBMITTED_BY_METHOD == 'POST'){
				...
				$id = $request->cleanData($request->post->getParam('id'));
				$id_user = $request->session->getSession('user_ref');
				$resultQuery = $database->deleteFromTable('DELETE FROM user_contact WHERE id = :id and id_user = :id_user LIMIT 1', array('id' => $id, 'id_user' => $id_user));
				$template = $this->container->get('TemplateTwig');
